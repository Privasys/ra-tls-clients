...
- `report_data_deterministic.json`: `spki_der`, `quote_time`, expected `report_data`.
- `report_data_challenge.json`: `spki_der`, `context`, `hctx` (given, since it depends
  on a live key schedule), expected `report_data`, with and without `gpu_evidence`.
- `messages.json`: canonical request and response bodies for each mode, with the
  expected parse result and the expected rejection reason for malformed inputs (wrong
  `v`, wrong mode echo, context length).
- `exporter.json`: an `exporter_master_secret`, the label, a context, and the expected
  `hctx`, so the exporter step is checked independently of a TLS stack (RFC 8446
  section 7.5 with SHA-256 and the 32-byte length).
